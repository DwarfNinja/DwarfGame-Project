...
onready var L_DOWN_ROOM = get_node("HouseRooms/L_Down")
onready var L_UP_ROOM = get_node("HouseRooms/L_Up")
onready var R_DOWN_ROOM = get_node("HouseRooms/R_Down")
onready var R_UP_ROOM = get_node("HouseRooms/R_Up")
...
func set_first_room():
	...
	var first_room_shapearea = first_room.get_node("ShapeArea")
	...
	for cell in first_room_shapearea.get_used_cells():
		$ShapeArea.set_cell(cell.x + first_room_pos.x, cell.y + first_room_pos.y, first_room_shapearea.get_cellv(cell), 
		false, false, false)
	rooms += 1
	first_room_placed = true
		
		
func set_random_room(random_room_walls, random_room_floor, random_room_shapearea, new_random_room_location):
	...
	for cell in random_room_shapearea.get_used_cells():
		$ShapeArea.set_cell(cell.x + get("room_pos_" + str(new_random_room_location)).x, 
		cell.y + get("room_pos_" + str(new_random_room_location)).y, random_room_shapearea.get_cellv(cell), 
		false, false, false, random_room_shapearea.get_cell_autotile_coord(cell.x, cell.y))
	rooms += 1
...
func check_randomroom_viability():
	...
	var random_room_shapearea = random_room.get_node("ShapeArea")
	...
	if count == 11 and possible_new_room_locations != []:
		...
		set_random_room(random_room_walls, random_room_floor, random_room_shapearea, chosen_randomroom_location)
...
func set_shape():
	...
	for cell in $ShapeArea.get_used_cells():
		var shape_width_extent = $ShapeArea.get_cell(cell.x + (random_shape_width -1), cell.y)
		var shape_height_extent = $ShapeArea.get_cell(cell.x, cell.y + (random_shape_height -1))
		var shape_rect_sw_corner = $ShapeArea.get_cell(cell.x + (random_shape_height -1), cell.y + (random_shape_height -1))
		...
		# Checks certain cells in $ShapeArea and $Walls if it is able the place the shape
...
func clear_used_shapearea():
	for cell in $Walls.get_used_cells():
		if $Walls.get_cellv(cell) == 16:
			$ShapeArea.set_cell(cell.x, cell.y, -1, 
			false, false, false)
